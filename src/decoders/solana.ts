...
export class SolanaDecoder extends BaseDecoder {
  ...
  decode(rawData: string): DecodedTransaction {
    try {
      ...
      const pubkey3 = buffer.subarray(offset, offset + 32);
      offset += 32;
      ...
      const decodedTx: DecodedTransaction = {
        ...
        chainSpecificData: {
          version,
          pubkeys: [
            this.toBase58(pubkey1),
            this.toBase58(pubkey2),
            this.toBase58(pubkey3)
          ],
      ...
      if (buffer.length >= offset + 12) {
        ...
        if (instructionType === 2) {
          ...
          // Based on our analysis, pubkey1 is sender, pubkey2 is recipient
