...
export class TronDecoder extends BaseDecoder {
  ...
  async decode(rawData: string): Promise<DecodedTransaction> {
    try {
      ...
      throw new Error(
        `Failed to decode Tron transaction: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private decodeTokenTransfer(
    _txData: { contractType?: string; contracts: unknown[] },
    rawData: string
  ): DecodedTransaction {
